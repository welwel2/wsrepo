...
def get_metadata_basenames(addon_metadata):
    try:
        ...
        print ('get_metadata_basenames failed')
        raise
...
def copy_metadata_files(source_folder, addon_target_folder, addon_metadata):
    #print ('copy_metadata_files source %s target %s' %(source_folder, addon_target_folder))
    for (source_basename, target_basename) in get_metadata_basenames(
            addon_metadata):
        #print ('source basename %s'%source_basename)
        source_path = os.path.join(source_folder, source_basename)
        #print ('before isfile %s'%source_path)
        try:
            ...
            print ('isfile failed for %s'%source_path)
            raise
        #print ('copied %s'%source_basename)
    #print ('end of copy_metadata_files')
...
def fetch_addon_from_git(addon_location, target_folder):
    ...
    try:
        ...
        #print ('create compressed addon archive\n')
        ...
        with open(archive_path, 'wb') as archive:
            cloned.archive(
                ...
                format='zip')
            print ('archived at %s' %archive_path)
        generate_checksum(archive_path)
        print ('checksum generated for %s' %archive_path)

        copy_metadata_files(
            clone_source_folder, addon_target_folder, addon_metadata)
        print ('end fetch_addon_from_git')
        return addon_metadata
    except:
        print ('fetch addon from git failed')
        raise
    finally:
        #print ('remove tree')
        try:
...
def fetch_addon_from_folder(raw_addon_location, target_folder):
    ...
    if addon_location != addon_target_folder:
        print ('copying metadata files')
        copy_metadata_files(
            addon_location, addon_target_folder, addon_metadata)
        #print ('after copy_metadata_files, %s'%addon_metadata)
    return addon_metadata
...
def fetch_addon(addon_location, target_folder, result_slot):
    try:
        if is_url(addon_location):
            print ('fetch_addon_from_git starting ...')
            addon_metadata = fetch_addon_from_git(
                addon_location, target_folder)
            print ('fetch_addon_from_git completed')
            ...
            print ('fetch_addon = path not found')
            raise RuntimeError('Path not found: ' + addon_location)
        print ('result_slot %s addon_metadata %s'%(result_slot, addon_metadata))
        result_slot.append(WorkerResult(addon_metadata, None))
    except:
        print ('exception in fetch addon')
...
def create_repository(
    ...
    metadata = []
    #print ('workers are here' )
    for worker in workers:
        #print ('curent worker ', worker)
        try:
